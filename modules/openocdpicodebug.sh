buildopenocdpicodebug() {
  ...
  if [ -f $INSTALLDIR/openocd-picodebug${EXEEXT} ]; then
    echo "Skipping $OPENOCDPICODEBUGVERSION for target $TARGET..."
    return 0
  else
    echo "Building $OPENOCDPICODEBUGVERSION for target $TARGET..."
  fi
  (
    [ -d $OPENOCDPICODEBUGVERSION ] && rm -rf $OPENOCDPICODEBUGVERSION
    ...
    CONFIGUREFLAGS="--enable-static --disable-shared --enable-cmsis-dap --disable-werror"
    ...
    mkdir -p $INSTALLDIR
    for file in openocd-picodebug${EXEEXT} ; do
    ...
    cp $BUILDDIR/$PREFIXDIR/share/openocd/scripts/interface/cmsis-dap.cfg $INSTALLDIR/../share/openocd/scripts/interface/
    ...
    cat >$INSTALLDIR/../share/openocd/scripts/board/picodebug.cfg <<EOF
